...
class GetRSS {
  static Future<List<Noticias>> gettrss() async {
    ...
    List<Noticias> noticias = new List(); //listado de noticias que se devuelven a la interfaz
    ...
    for (int i = 0; i < rssFeed.items.length; i++) {
      ...
      noticias.add(new Noticias(rssFeed.items[i].title, rssFeed.items[i].description, rssFeed.items[i].pubDate, rssFeed.items[i].enclosure.url, rssFeed.items[i].link));
     }
    return noticias;
  }
}
...
class Noticias{
  ...
  Noticias(this._titulo, this._descripcion, this._fecha, this._urlImage, this._urlNoticias);
